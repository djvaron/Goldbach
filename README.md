...
The optimized parallel code uses gangs and vectors to parallelize the code among more threads. The default thread count is 128; by increasing that thread count by distributing the work among gangs and then threads, the code is further parallelized. Because the problem is embarrassingly parallel, we see a substantial increase in performance as seen by the blue trianges (optimized) versus red squares (unoptimized).  <br />
